...
export function cacheGet(key) {
  ...
  store.delete(key); // promote to most-recently-used (Map keeps insertion order)
  store.set(key, e);
  return e.value;
}
...
export function cacheSet(key, value, ttlMs) {
  ...
  if (store.size > 500) store.delete(store.keys().next().value); // evict LRU (oldest entry)
}
...
export function rateLimit(ip, max = 12, windowMs = 60_000) {
  ...
  b.count++;
  buckets.set(ip, b);
  // Bound memory: drop the oldest IP once the map gets large (it just resets its window).
